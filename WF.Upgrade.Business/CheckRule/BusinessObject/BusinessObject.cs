...
namespace WF.Upgrade.Business
{
    ...
    [CheckRuleRemark("流程模版文件中字段在业务对象中不存在,DomainXML中存在重复的字段")]
    [CheckRuleKind("数据检测")]
    ...
    public class BusinessObject : ICheckRule
    {
        public CheckRule Check(object input)
        {
            ...
            string rootPath = Site.GetSiteInfoEntity().UpFilesAddress + @"\WorkflowDoc\流程模板\";
            //@"E:\workflow\07 项目ERP\04 重庆融汇\WorkflowDoc\流程模板";
        ...
        {
            string sql = @"select OBJECT_ID('fn_GetProcessKindPath') as 'ProcessKindPath'";
            DataTable dt = CPQuery.From(sql).FillDataTable();
            if (dt.Rows[0]["ProcessKindPath"].ToString() == "") {
                string sql_fn = @"CREATE FUNCTION	[dbo].[fn_GetProcessKindPath](@ProcessKindGUID uniqueidentifier)
RETURNS varchar(MAX)
AS
BEGIN
     ...
     SET @retVal ='';
     SET @parentGUID = @ProcessKindGUID;
     WHILE(@parentGUID IS NOT NULL)
     BEGIN
       SELECT TOP 1 @retVal =name+'》'+@retVal,@parentGUID =ParentGUID FROM myWorkflowProcessKind WHERE ProcessKindGUID = @parentGUID
     END
     --去掉最后的'》'
     IF(LEN(@retVal)>0)
     BEGIN
...
";
                CPQuery.From(sql_fn).ExecuteNonQuery();
            }
